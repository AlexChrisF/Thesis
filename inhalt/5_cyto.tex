...
any data to a server. This helps both, privacy concerns as well as performance. The following sections shall explain how CYTO AI works and which techniques and architectures were used to build it.\index{rich client}
...
images will be categorized. \index{automatically classifying}
...
annotate. Newly labeled images will stay visible.\index{Filtering and sorting}
...
It needs to provide integrated Git support as well as easy debugging, syntax highlighting, and an intelligent code completion. \index{IDE}
...
package manager \texttt{npm} is used. \index{npm} \index{Node}
...
\lstinputlisting[caption=Categories component, label=lst:categories]{code/cyto/categoryList.js}
...
where \texttt{dataset} is an instance for handling the data, and \texttt{imageTags} are the HTML image tags. The API is using a CNN provided over a Content Delivery Network (CDN).
