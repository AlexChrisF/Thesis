...
CYTO AI is the first fully web-based image labeling and classifying tool publicly available. 

Based on the JavaScript frameworks React and TensorFlow.js,
...
any data to a server. This helps both, privacy concerns as well as performance.
...
%There are several ways to annoate an image. It is possible %to drag an image and drop it on the wished category. A %different way is to click on one picture in order to select %it an then use the keyboard to annotate the image. %Following keys are supported to annotate an image:
