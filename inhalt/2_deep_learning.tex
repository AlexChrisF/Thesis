...
Deep learning is a class of neural network optimization methods for networks that have
 a significant number of hidden layers between their input and output layer. \index{neural network} 
...
A feed-forward network comprises one input layer, H hidden layers and one output layer. \index{feed-forward neural network}
...
Training a neural network means iteratively applying a forward pass followed by an error backpropagation. \index{backpropagation}
During the forward pass the outputs for each neuron are calculated based on its inputs. \index{Backpropagation}
...
Calculating the gradient for a complete dataset can take a lot of computation time.\index{weights}
...
convolutional neural networks (CNN). \index{classification}
...
In a convolutional layer several kernels are applied to extract spacial related features. Each output from the the convolutional layer is called a feature-map. Each feature-map was created by a different convolutional kernel of the layer. \index{convolutional layer}
...
selects the maximum of each window. \index{pooling layer}
