...
Given a certain input, a forward pass calculates the output of each unit . A predefined cost function \(E\) then compares the resulting outputs \(yout\) with the correct answer and determines the error derivatives. Common cost cost functions are mean squared error or crossentropy loss.
