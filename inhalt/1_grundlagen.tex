...
Often data needs to be aggregated from multiple child components. Then makes sense to lift all states up from to the parent component, the parent component then passes down the individual states for the child components.
...
The Square component is no longer keeping its own state, it receives the value from its parent Board and informs the parent when it was clicked. These components are called controlled components. 
