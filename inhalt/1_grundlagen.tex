...
Using the calculated gradient \(\Delta_{W}E(y_{out})\)it is now possible to determine next update for a specific weight:

\begin{equation}
\(
W_{l}^{t+1}=
W_{l}^{t}-n\Delta_{W_{l}^{t}}E(y_{yout)}
\)
\end{equation}

Where \(W^{t}\), \(W^{t+1}\) are the current weights and the weight matrix for the updated weights. \(n\) is the learning rate.
