...
\( y_{i} \) is the output of the neuron from the current layer. The output is transformed with a non-linear activation function, z is called the preactivation and w denotes the edge weights. \( x_{i} \) is the output of a unit from the previous layer. 
At every layer first the total input z is computed. Z is the weighted sum of the outputs from the layer before. After that a non linear function \( f(...) \)f() is applied to z in order to get the output \( y_{j} \) of the unit
